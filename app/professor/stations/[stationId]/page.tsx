...
export default function ProfessorStationDetailPage({
  ...
  const fetchStationData = async (isManual = false) => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        // Set Station Criteria (public.station_criteria)
        if (json.criteria) {
          setCriteria(json.criteria)
        } else {
          const targetStationId = json.station?.id || stationId
          getStationCriteriaAction(targetStationId).then((cRes) => {
  ...
  const handleCreateCriteria = async (e: React.FormEvent) => {
    ...
    const numPoints = Number(criteriaPoints)
    if (isNaN(numPoints) || numPoints === 0) {
      showError('Please enter a valid deduction point amount.')
      return
    }
    ...
    try {
      ...
      if (res.success && res.criterion) {
        showSuccess('Preset penalty added successfully.')
        ...
        showError(res.error || 'Failed to add preset penalty.')
      }
    } catch (err: any) {
      showError(err?.message || 'Server error creating preset penalty.')
    } finally {
  ...
  const handleDeleteCriteria = async (criterionId: string) => {
    ...
    try {
      const res = await deleteStationCriterionAction(criterionId, station.id)
      if (res.success) {
        showSuccess('Preset penalty removed.')
        setCriteria((prev) => prev.filter((c) => c.id !== criterionId))
      } else {
        showError(res.error || 'Failed to remove preset penalty.')
      }
    } catch (err: any) {
      showError(err?.message || 'Server error removing preset penalty.')
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center p-16 rounded-3xl bg-white/50 dark:bg-slate-900/50 border border-slate-200/80 dark:border-slate-800">
          ...
          <p className="text-xs font-bold text-slate-500">Loading station checklist & criteria...</p>
        </div>
      ) : !station ? (
        ...
        <>
          ...
          {/* Section 2: Preset Clinical Deductions & Penalties (public.station_criteria where station_id = stationId) */}
          <div className="space-y-4 pt-4 border-t border-slate-200/80 dark:border-slate-800">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-rose-600 to-red-600 text-white text-xs font-bold shadow-md shadow-rose-500/25 hover:from-rose-700 hover:to-red-700 transition-all active:scale-[0.98] cursor-pointer"
              >
                <Plus className="size-4" />
            ...
            {criteria.length === 0 ? (
              <div className="p-8 rounded-3xl bg-rose-50/20 dark:bg-rose-950/10 border border-dashed border-rose-200/80 dark:border-rose-900/40 text-center space-y-2">
                ...
                <button
                  onClick={() => setIsCreateCriteriaOpen(true)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 bg-rose-50/50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 transition-colors cursor-pointer"
                >
      ...
      {isQuestionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-xl rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-5 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            ...
            <form onSubmit={handleSubmitQuestion} className="space-y-4">
              ...
              <div className="space-y-1.5" ref={typeDropdownRef}>
                ...
                <div className="relative">
                  ...
                  {typeDropdownOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 p-2 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl z-50 space-y-1 animate-in fade-in slide-in-from-top-2">
      ...
      {/* ========================================================================= */}
      {/* MODAL 3: CREATE STATION PRESET CRITERIA MODAL (public.station_criteria)   */}
      {/* ========================================================================= */}
            ...
            <form onSubmit={handleCreateCriteria} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Failed Hand Hygiene / Asepsis Breach"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="e.g. Candidate omitted hand sanitization or PPE prior to patient examination"
                  rows={2}
                  className="w-full px-3.5 py-2.5 rounded-2xl text-xs bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-500/30 focus:border-rose-500"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                <p className="text-[10px] text-slate-400">
                  Values are automatically saved as negative deductions (e.g. 0.5 becomes -0.5 pts).
                </p>
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-rose-600 text-white text-xs font-bold shadow-md shadow-rose-500/25 hover:bg-rose-700 transition-all disabled:opacity-50 cursor-pointer"
                >
                  {submittingCriteria ? (
                    <>
                      <Loader2 className="size-4 animate-spin" />
