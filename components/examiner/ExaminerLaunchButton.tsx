...
interface ExaminerLaunchButtonProps {
  variant?: 'topbar' | 'dropdown' | 'hero' | 'compact' | 'sidebar'
  onClick?: () => void
  className?: string
}
...
export function ExaminerLaunchButton({
  ...
  if (variant === 'sidebar') {
    return (
      <Link
        href="/examiner"
        onClick={onClick}
        className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white ${className}`}
      >
  ...
  if (variant === 'hero') {
    return (
      ...
      >
        ...
        <span>Launch Examiner Station</span>
      </Link>
    )
  }
  ...
  return (
    ...
    >
      ...
      <span className="hidden sm:inline">Examiner Mode</span>
    </Link>
  )
}
