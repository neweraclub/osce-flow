...
export function TemplateQuickAssignDrawer({
  ...
  // Escape key handler for drawer dismissal
  useEffect(() => {
  ...
  }, [mode, isOpen, isFormModalOpen, onClose])
  ...
  // Inner Template Content (shared between Left Drawer & Left Sidebar inline mode)
  const templateBody = (
    <div
      className={`flex flex-col h-full bg-white dark:bg-slate-900 ${
        mode === 'drawer'
          ? 'w-screen max-w-md sm:w-[420px] border-r border-slate-200 dark:border-slate-800 shadow-2xl pointer-events-auto animate-in slide-in-from-left duration-300'
          : 'w-full'
      }`}
    >
      {/* Header */}
      <div className="p-3.5 sm:p-4 border-b border-slate-100 dark:border-slate-800/80 bg-slate-50/70 dark:bg-slate-950/60 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="size-8 rounded-xl bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center shadow-xs">
            <Zap className="size-4 fill-amber-500/30" />
          </div>
          <div>
            ...
            <p className="text-[10px] sm:text-[11px] text-slate-400 leading-tight">
              Drag right <ArrowRight className="inline size-3 text-amber-500" /> onto candidate card or click Apply
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold shadow-xs shadow-amber-500/20 transition-all cursor-pointer active:scale-95"
      ...
      {activeStudentName ? (
        <div className="px-3.5 py-2 bg-amber-50/70 dark:bg-amber-950/40 border-b border-amber-200/60 dark:border-amber-900/40 flex items-center justify-between text-xs shrink-0">
          <div className="flex items-center gap-1.5 truncate">
            <span className="text-slate-500 dark:text-slate-400 font-medium text-[11px]">Active:</span>
            <span className="font-bold text-amber-700 dark:text-amber-300 truncate text-[11px]">
              {activeStudentName}
            </span>
          </div>
          <span className="text-[9px] font-mono font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider shrink-0 bg-amber-100/70 dark:bg-amber-900/50 px-1.5 py-0.5 rounded border border-amber-200/50 dark:border-amber-800/50">
            Target Ready
          </span>
        </div>
      ) : (
        <div className="px-3.5 py-2 bg-slate-100/60 dark:bg-slate-800/40 border-b border-slate-200/60 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 shrink-0">
          Select a candidate to drag templates directly onto their marksheet.
        </div>
      )}

      {/* Filter Tabs & Search Bar */}
      <div className="p-3 border-b border-slate-100 dark:border-slate-800 space-y-2.5 shrink-0 bg-slate-50/50 dark:bg-slate-900/40">
        {/* Type Filter Pills */}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 rounded-xl text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-amber-500/40"
          />
        </div>
      </div>
        ...
        {filteredTemplates.length === 0 ? (
          <div className="py-12 px-4 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 space-y-2">
            ...
            <p className="text-[11px] text-slate-400 max-w-xs mx-auto">
              Click <strong>Add</strong> above to create reusable bonus and deduction presets.
            </p>
          </div>
        ) : (
          filteredTemplates.map((tpl) => {
            ...
            return (
              <div
                ...
                className={`p-3 rounded-2xl border transition-all duration-200 group flex flex-col justify-between space-y-2.5 cursor-grab active:cursor-grabbing select-none hover:shadow-md ${
                  isBonus
                    ? 'bg-emerald-50/40 dark:bg-emerald-950/20 border-emerald-200/70 dark:border-emerald-800/40 hover:border-emerald-400 dark:hover:border-emerald-600 shadow-emerald-500/5'
                    : 'bg-rose-50/40 dark:bg-rose-950/20 border-rose-200/70 dark:border-rose-800/40 hover:border-rose-400 dark:hover:border-rose-600 shadow-rose-500/5'
                }`}
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2 min-w-0">
                    <div
                      className="p-1 rounded-md text-slate-400 group-hover:text-amber-500 dark:group-hover:text-amber-400 shrink-0 mt-0.5"
                      title="Drag this card onto the candidate card on the right"
                    >
                ...
                <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      title="Adjust points or add clinical notes before saving"
                    >
      ...
      {/* Footer Notice */}
      <div className="p-3 bg-slate-50 dark:bg-slate-950/80 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-400 flex items-center justify-between shrink-0">
        <span className="flex items-center gap-1 text-[10px] sm:text-[11px] font-medium text-slate-500 dark:text-slate-400">
          ...
          <span>Drag card rightwards onto candidate</span>
        </span>
        {mode === 'drawer' && onClose && (
          <button
            type="button"
  ...
  return (
    <>
      {mode === 'drawer' ? (
        /* Floating Left Drawer without blocking backdrop overlay */
        <div className="fixed inset-y-0 left-0 z-50 flex max-w-full pointer-events-none">
          {templateBody}
        </div>
      ) : (
        /* Inline inside left sidebar */
        templateBody
      )}
      ...
      {isFormModalOpen && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="relative w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden p-6 space-y-5 animate-in zoom-in-95 pointer-events-auto">
